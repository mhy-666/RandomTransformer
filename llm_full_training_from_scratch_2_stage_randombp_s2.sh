...
# 每个checkpoint只测试：继续相同的random BP 或 切换到standard training
# ============================================================================
...
STAGE1_OUTPUT_DIR="${BASE_DIR}/outputs/freeze_random_bp"
OUTPUT_DIR="${BASE_DIR}/outputs/freeze_random_bp_stage2"
...
echo "  - Continue with same random BP (fixed mode)"
echo "  - Continue with same random BP (resample mode)"
...
for exp_key in "${!EXPERIMENTS[@]}"; do
    IFS=':' read -r stage1_base s2_random_bp base_description <<< "${EXPERIMENTS[$exp_key]}"
    ...
    for stage1_mode in "fixed" "resample"; do
        ...
        if [ "$s2_random_bp" = "none" ]; then
            modes=("standard")
        else
            # 如果继续用random BP，测试fixed和resample
        ...
        for s2_mode in "${modes[@]}"; do
            if [ "$s2_mode" = "standard" ]; then
                ...
                resample_flag=""
                mode_desc="standard training (no random BP)"
            elif [ "$s2_mode" = "resample" ]; then
                exp_name="s2_from_${stage1_exp}_to_${s2_random_bp}_resample"
                resample_flag="--resample_every_batch"
                mode_desc="continue RBP on ${s2_random_bp} (resample)"
            else
                exp_name="s2_from_${stage1_exp}_to_${s2_random_bp}_fixed"
                resample_flag=""
                mode_desc="continue RBP on ${s2_random_bp} (fixed)"
            fi
            ...
            for seed in "${SEEDS[@]}"; do
                ...
                if [ "$s2_random_bp" = "none" ]; then
                    ...
                    random_bp_params="--random_backprop_strategy full_random"
                    random_bp_params="${random_bp_params} --apply_random_backprop_to_layers ${s2_random_bp}"
                    random_bp_params="${random_bp_params} --projection_type random"
                    random_bp_params="${random_bp_params} ${resample_flag}"
                fi
...
python experiment_llm.py \
    ...
    --project_name "freeze_random_bp_stage2" \
    --run_name "${job_name}" \
    ${random_bp_params}
...
echo "========================================================================"
echo "All ${exp_count} Stage 2 jobs submitted!"
echo ""
echo "Experiment Structure:"
echo ""
echo "Group 1: From freeze_attn_rbp_attn (fixed & resample)"
echo "  → Continue RBP on Attn (fixed/resample) + Standard training"
echo ""
echo "Group 2: From freeze_mlp_rbp_mlp (fixed & resample)"
echo "  → Continue RBP on MLP (fixed/resample) + Standard training"
echo ""
echo "Group 3: From freeze_both_rbp_both (fixed & resample)"
echo "  → Continue RBP on Both (fixed/resample) + Standard training"
echo ""
echo "Research Questions:"
echo "  1. Does continuing random BP help vs switching to standard?"
echo "  2. Does S1 mode (fixed/resample) affect S2 performance?"
echo "  3. Which freeze strategy in S1 leads to best S2 results?"
echo "  4. Fixed vs Resample in S2: consistency with S1 or switch?"
echo "========================================================================"
